...
- **Merge Logic**: The action intelligently preserves existing deployment entries. If an iFlow already exists in the deployment file, all its settings (including stage-specific flags like DEV-ENV, TEST-ENV, PROD-ENV) are retained. Only the `ArtifactName` is updated if it changed in the source.
